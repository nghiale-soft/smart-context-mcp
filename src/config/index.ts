...
export function loadConfig(): SmartContextConfig {
  ...
  return {
    ...
    lowAiModel: process.env.LOW_AI_MODEL || process.env.ANTHROPIC_MODEL || process.env.OPENAI_MODEL || fileConfig.lowAiModel || 'nemotron-3-super:cloud',
    lowAiBaseUrl: process.env.LOW_AI_BASE_URL || process.env.ANTHROPIC_BASE_URL || process.env.OPENAI_BASE_URL || fileConfig.lowAiBaseUrl || 'http://localhost:11434',
    lowAiAuthToken: process.env.LOW_AI_AUTH_TOKEN || process.env.ANTHROPIC_AUTH_TOKEN || fileConfig.lowAiAuthToken || 'ollama',
